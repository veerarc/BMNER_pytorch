...
parameters['word_lstm_size'] = args.word_lstm_size
...
args.embedding = True
args.hidden_size=parameters['word_lstm_size']
...
def evaluating(model, datas, best_F, predf=None):
    ...
    for data in datas:
        ...
        if use_gpu:
            ...
            val, out = model(dwords, chars2_mask, dcaps, chars2_length, d, gaz_ids, pos_ids, pre_ids, suf_ids)
        predicted_id = out
...
for epoch in range(1, args.nb_epoch):
    ...
    for i in range(len(train_data)):
        ...
        if count % len(train_data) == 0:
            ...
            os.system('sh %s' % (args.clef_eval_script))
